...
function AssistantTransactionManagement() {
    ...
    <div className="w-full">
        ...
        <div className="mb-6 bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-start gap-3 shadow-sm">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700 mt-1">
              บริษัทของคุณยังไม่มีแพ็คเกจที่ใช้งานอยู่ หรือแพ็คเกจหมดอายุแล้ว คุณสามารถดูประวัติการทำธุรกรรมได้ แต่จะไม่สามารถเพิ่ม แก้ไข หรือลบข้อมูลได้
            </p>
            <p className="mt-3 text-sm font-bold text-amber-900 bg-amber-100/50 w-fit px-3 py-1.5 rounded-lg border border-amber-200">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">ประวัติธุรกรรม (ผู้ช่วย)</h1>
            <p className="text-sm text-gray-500">จัดการและบันทึกรายรับ-รายจ่ายของบริษัท</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            <p className="text-sm font-medium text-gray-500 mb-1">รายรับเดือนนี้</p>
            <p className="text-2xl font-bold text-green-600">฿{summary.income.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          </div>
          <div className="p-3 bg-green-50 rounded-lg"><ArrowUpCircle size={24} className="text-green-600" /></div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            <p className="text-sm font-medium text-gray-500 mb-1">รายจ่ายเดือนนี้</p>
            <p className="text-2xl font-bold text-red-600">฿{summary.expense.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          </div>
          <div className="p-3 bg-red-50 rounded-lg"><ArrowDownCircle size={24} className="text-red-600" /></div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            <p className="text-sm font-medium text-gray-500 mb-1">ยอดคงเหลือ</p>
          ...
          <div className="p-3 bg-gray-50 rounded-lg"><Wallet size={24} className="text-gray-700" /></div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <select 
            ...
            className="border border-gray-300 rounded-lg px-3 py-2 outline-none focus:border-black focus:ring-1 focus:ring-black text-sm transition-all"
          >
            {[...Array(12)].map((_, i) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-200">
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">วันที่</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">รายการ</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">หมวดหมู่</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">จำนวนเงิน</th>
              ...
              {loading ? (
                ...
                <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    <span className="bg-gray-100/80 px-2.5 py-1 rounded-md text-xs border border-gray-200">{tx.category_name}</span>
                  </td>
                  <td className={`px-6 py-4 text-sm font-bold text-right whitespace-nowrap ${tx.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {isModalOpen && hasActivePackage && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative animate-in fade-in zoom-in duration-200">
            <button onClick={closeModal} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 transition-colors"><X size={20} /></button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative" ref={catDropdownRef}>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black outline-none text-sm transition-all"
                    required={!formData.category_id}
                  />
                  <button type="button" onClick={() => setIsCatDropdownOpen(!isCatDropdownOpen)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                      ...
                      <ul className="py-1">
                        {filteredCategoriesForDropdown.map((cat) => (
                          <li
                            ...
                            className={`flex items-center justify-between px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 transition-colors ${formData.category_id === cat.id ? "bg-gray-50 font-medium" : "text-gray-700"}`}
                          >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-black transition-all" 
                    required 
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-black transition-all" 
                  placeholder="เช่น ค่าอาหาร, ขายสินค้า..." 
                />
              </div>

              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={closeModal} className="px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">ยกเลิก</button>
                <button type="submit" disabled={isSaving || !formData.category_id} className="px-4 py-2.5 text-sm font-medium text-white bg-black rounded-lg hover:bg-gray-800 transition-colors flex items-center gap-2 disabled:opacity-50">
...
export default function AssistantTransactionsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-gray-400" size={40} /></div>}>
      <AssistantTransactionManagement />
    </Suspense>
  );
}